// 5-1-1.cpp : 이 파일에는 'main' 함수가 포함됩니다. 거기서 프로그램 실행이 시작되고 종료됩니다.
//
...
public:
    ...
    NameCard(const NameCard& src) : pos(src.pos)
    {
        name = new char[strlen(src.name) + 1];
        comp = new char[strlen(src.comp) + 1];
        phone = new char[strlen(src.phone) + 1];
        strcpy(name, src.name);
        strcpy(comp, src.comp);
...
int main()
{
    ...
    NameCard dest1 = manClerk;
    NameCard manSenior("Ponyo", "AppleComp", "010-4983-7173", COMP_POS::SENIOR);
    NameCard dest2 = manSenior;

    dest1.ShowNameCardInfo();
    dest2.ShowNameCardInfo();
    return 0;
}
